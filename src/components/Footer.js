...
const Footer = () => {
  ...
  return (
    <div className="fixed bottom-2 w-full max-w-md bg-[#131a10] rounded-full flex justify-around items-center border border-gray-700 shadow-md" style={{zIndex:10}}>
      ...
      <button className={`cursor-pointer hover:text-gray-600 ${location.pathname === "/reward" ? "p-4 rounded-foot shadow-xl bg-green-400" : ""}`} style={{padding:8, margin:6}}>
          <Gift className={`${location.pathname === "/reward" ? "text-black" : "text-gray-400"}`} size={24} />
        </button>
      </Link>
