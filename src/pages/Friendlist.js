  ...
  return (
    <div className="min-h-screen imgbg text-white px-4 pt-6 pb-24 w-full max-w-md mx-auto font-sans">
      <div className="flex items-center mb-6">
        <button onClick={() => navigate("/miningTeam")} className="p-2 w-10 h-10 rounded-xl bg-apin border border-[#1efcb9]/20 flex items-center justify-center shadow-md" >
          <FaArrowLeft size={18} className="text-[#1efcb9]" />
        </button>
        <h1 className="flex-grow text-center text-xl font-light tracking-widest" style={{ paddingRight: '36px' }}> Team List</h1>
      </div>
            ...
            <div key={member.id} className="flex justify-between items-center border-b border-white/10 pb-4">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 bg-apin text-[#1efcb9] rounded-md flex items-center justify-center text-lg">
                  ...
                  />              </div>
                <div>
                  <p className="text-white font-semibold text-sm mb-0.5"> {member.name || "Unknown"}</p>
                  <p className="text-xs text-gray-400 font-mono">{formatDate(member.created_at)}</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-white font-bold text-sm"> {member.telegram_id || "N/A"}</p>
                <div className="flex items-center justify-end gap-1">
                  <span className="text-[10px] text-gray-400">Telegram ID  </span>
